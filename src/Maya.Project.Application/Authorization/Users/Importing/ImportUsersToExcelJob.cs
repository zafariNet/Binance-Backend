            ...
            List<ImportUserDto> invalidUsers)
        {
            ...
            else
            {
                await _appNotifier.SendMessageAsync(
                    args.User,
                    new LocalizableString("AllUsersSuccessfullyImportedFromExcel", ProjectConsts.LocalizationSourceName),
                    null,
            ...
            using (var uow = _unitOfWorkManager.Begin())
            {
                using (CurrentUnitOfWork.SetTenantId(args.TenantId))
                {
                    AsyncHelper.RunSync(() => _appNotifier.SendMessageAsync(
                        args.User,
                        new LocalizableString("FileCantBeConvertedToUserList", ProjectConsts.LocalizationSourceName),
                        null,
